...
class Usuario(TimestampMixin, Base):
    ...
    perfil: Mapped[Perfil] = mapped_column(Enum(Perfil, native_enum=False, create_constraint=True))
...
class Veiculo(TimestampMixin, Base):
    ...
    tipo: Mapped[TipoVeiculo] = mapped_column(Enum(TipoVeiculo, native_enum=False, create_constraint=True))
    status: Mapped[StatusVeiculo] = mapped_column(Enum(StatusVeiculo, native_enum=False, create_constraint=True), default=StatusVeiculo.DISPONIVEL)
...
class Rota(TimestampMixin, Base):
    ...
    motorista_id: Mapped[int | None] = mapped_column(ForeignKey("usuarios.id"), index=True)
    status: Mapped[StatusRota] = mapped_column(Enum(StatusRota, native_enum=False, create_constraint=True), default=StatusRota.PLANEJADA)
...
class RotaAlternativa(TimestampMixin, Base):
    ...
    criterio: Mapped[CriterioAlternativaRota] = mapped_column(Enum(CriterioAlternativaRota, native_enum=False, create_constraint=True), nullable=False)
...
class RotaEntrega(Base):
    ...
    prioridade: Mapped[Prioridade | None] = mapped_column(Enum(Prioridade, native_enum=False, create_constraint=True))
...
class RotaHistorico(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    rota_id: Mapped[int] = mapped_column(ForeignKey("rotas.id"), index=True)
    evento: Mapped[TipoEventoRota] = mapped_column(Enum(TipoEventoRota, native_enum=False, create_constraint=True))
...
class Pedido(TimestampMixin, Base):
    ...
    status: Mapped[StatusPedido] = mapped_column(Enum(StatusPedido, native_enum=False, create_constraint=True), default=StatusPedido.ABERTO)
    prioridade: Mapped[Prioridade] = mapped_column(Enum(Prioridade, native_enum=False, create_constraint=True), default=Prioridade.NORMAL)
...
class Entrega(TimestampMixin, Base):
    ...
    status: Mapped[StatusEntrega] = mapped_column(Enum(StatusEntrega, native_enum=False, create_constraint=True), default=StatusEntrega.AGUARDANDO_COLETA)
